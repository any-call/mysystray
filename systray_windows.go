...
func registerSystray() {
	if err := wt.initInstance(); err != nil {
		mylog.Errorf("Unable to init instance: %v", err)
		return
	}

	if err := wt.createMenu(); err != nil {
		mylog.Errorf("Unable to create menu: %v", err)
		return
	}
...
func nativeLoop() {
	...
	for {
		...
		switch int32(ret) {
		case -1:
			mylog.Errorf("Error at message loop: %v", err)
			return
		case 0:
			return
		default:
...
func SetIcon(iconBytes []byte) {
	...
	if err != nil {
		mylog.Errorf("Unable to write icon data to temp file: %v", err)
		return
	}
	if err := wt.setIcon(iconFilePath); err != nil {
		mylog.Errorf("Unable to set icon: %v", err)
		return
	}
}
...
func (item *MenuItem) SetIcon(iconBytes []byte) {
	...
	if err != nil {
		mylog.Errorf("Unable to load icon from temp file: %v", err)
		return
	}

	h, err = wt.iconToBitmap(h)
	if err != nil {
		mylog.Errorf("Unable to convert icon to bitmap: %v", err)
		return
	}
	...
	if err != nil {
		mylog.Errorf("Unable to addOrUpdateMenuItem: %v", err)
		return
	}
}
...
func SetTooltip(tooltip string) {
	if err := wt.setTooltip(tooltip); err != nil {
		mylog.Errorf("Unable to set tooltip: %v", err)
		return
	}
}
...
func addSeparator(id uint32) {
	...
	if err != nil {
		mylog.Errorf("Unable to addSeparator: %v", err)
		return
	}
}
...
func hideMenuItem(item *MenuItem) {
	...
	if err != nil {
		mylog.Errorf("Unable to hideMenuItem: %v", err)
		return
	}
}
